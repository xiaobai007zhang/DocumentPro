...
QtDrawingPaperEditor::QtDrawingPaperEditor(QWidget* parent, int width, int height) : QLabel(parent)
{
...
double QtDrawingPaperEditor::CalcPaperWidthOfPerPixel(double scale_value, int paper_width, int widget_width)
{
    ...
    if (paper_width_of_per_pixel < 0.0005)
        paper_width_of_per_pixel = 0.0005;
...
void QtDrawingPaperEditor::OnWheelValueChanged(QPoint mouse_pos, QPoint step)
{
    ...
    if (draw_paper_rect_.contains(mouse_pos))
    {
        ...
        if (scale_value_ > SCALE_VALUE_MAX)
            scale_value_ = SCALE_VALUE_MAX;
        if (scale_value_ < SCALE_VALUE_MIN)
            scale_value_ = SCALE_VALUE_MIN;
...
void QtDrawingPaperEditor::DrawPaper(QPainter& painter)
{
    ...
    {
    case TOHEIGHT:
        draw_height = height() * scale_value_;

        draw_width = PaperWidth2DrawWidth(paper_width_);
        break;
    case TOWIDTH:
        draw_height = PaperWidth2DrawWidth(paper_height_);

        draw_width = width() * scale_value_;
        break;
    case TOKEEPRATIO:
        draw_height = m_pixmap->height() * scale_value_;

        draw_width = m_pixmap->width() * scale_value_;
        break;
    case TOEXPAND:
        draw_height = height() * scale_value_;
    ...
    // int draw_height = height() * scale_value_;

    // int draw_width = width() * scale_value_;
    ...
    painter.drawPixmap(draw_paper_rect_, *m_pixmap);
    // painter.drawPixmap(QPoint(draw_paper_rect_.x(), draw_paper_rect_.y()), *pixmap);
}
...
void QtDrawingPaperEditor::slot_okBtnClick(PixStatus flag)
{
    switch (flag)
    {
    case TOHEIGHT:
